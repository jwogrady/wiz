...
test_dry_run_execution() {
    ...
    output=$(timeout "$TEST_TIMEOUT" "$INSTALLER" --dry-run --skip-identity 2>&1) || exit_code=$?
    ...
    if echo "$output" | grep -qi "dry-run\|DRY-RUN"; then
        ...
        fail "Dry-run mode executed but output doesn't contain expected indicators"
        echo "$output" | head -30
        return 1
    fi
}
...
test_no_changes() {
    ...
    if [[ "$git_config_before" == "$git_config_after" ]] && [[ "$ssh_keys_before" == "$ssh_keys_after" ]]; then
