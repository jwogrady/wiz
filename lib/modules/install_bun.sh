...
install_bun() {
    ...
    # Bun does not publish a checksum for install.sh — pass empty SHA to warn and proceed.
    local bun_tmp
    bun_tmp="$(wiz_download_verified \
        "https://bun.sh/install" "" \
        "Failed to download Bun installer")" || module_fail "Bun installation failed"
    ...
    # Normalize any old-format Bun init in .bashrc written by the official installer
    # to our canonical block format before appending ours. Only applies to .bashrc
    # since the official installer does not touch .zshrc.
    if [[ ${WIZ_DRY_RUN:-0} -eq 0 ]]; then
        if [[ -f "$HOME/.bashrc" ]]; then
            if grep -q "BUN_INSTALL" "$HOME/.bashrc" 2>/dev/null && \
               ! grep -q ">>> Bun init >>>" "$HOME/.bashrc" 2>/dev/null; then
                sed_inplace \
    ...
    wiz_add_shell_block ">>> Bun init >>>" "$_BUN_SHELL_BLOCK"
