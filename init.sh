...
# -----------------------------------------------------------------------------
# WIZ Environment Initialization Script (Consolidated)
# -----------------------------------------------------------------------------
...
prompt() {
    ...
    while true; do
        ...
        [[ "$val" =~ $regex ]] && break || echo "Invalid input. Please try again."
    done
    eval "$var=\"\$val\""
}
...
extract_keys() {
    ...
    if [ -f "$KEYS_SRC" ]; then
        ...
        tar -xzvf "$SCRIPT_DIR/keys.tar.gz" -C "$KEYS_DEST"
...
# --- Setup Global Git Config and .gitignore ---
setup_gitconfig() {
...
EOF
    ...
    if [[ -f "$ENV_FILE" ]]; then
        ...
        [[ $(stat -c "%a" "$ENV_FILE") == "600" ]] && echo -e "\e[32mPermissions are correct (600)\e[0m" || echo -e "\e[31mPermissions are incorrect. Please set to 600.\e[0m"
    else
    ...
    if [[ -d "$HOME/.ssh" && "$(ls -A $HOME/.ssh 2>/dev/null)" ]]; then
        ...
        for key in "$HOME/.ssh/"id_*; do
            ...
            if [[ -f "$key" ]]; then
                echo -n "  - $(basename "$key"): "
                [[ $(stat -c "%a" "$key") == "600" ]] && echo -e "\e[32mPermissions are correct (600)\e[0m" || echo -e "\e[31mPermissions are incorrect. Please set to 600.\e[0m"
            fi
        done
    else
